...
// Import data struktur pemerintahan
...
// Import IconUserFilled dari Tabler Icons
import { IconUserFilled, IconArrowLeft, IconArrowRight, IconInfoSquareRounded } from '@tabler/icons-react';
// Import fungsi useEffect dan useState dari React
import { useEffect, useState } from 'react';

const ProfilDesa = () => {
	// State variable untuk menyimpan jumlah slide yang ditampilkan sesuai dengan ukuran layar
	const [slidesPerView, setSlidesPerView] = useState(5);

	// Fungsi untuk mengecek apakah ukuran layar saat ini adalah mobile view atau tidak
	const getMobileView = () => {
		return window.innerWidth <= 1024; // Adjust the width as per your requirement
	};

	// Mengatur jumlah slide yang ditampilkan berdasarkan ukuran layar
	// useEffect akan dijalankan setiap kali komponen ProfilDesa dibuka
	useEffect(() => {
		...
		handleResize(); // Set jumlah slide yang ditampilkan saat pertama kali membuka halaman
		window.addEventListener('resize', handleResize); // Menambahkan event listener untuk mengecek perubahan ukuran layar

		// Cleanup function untuk menghapus event listener saat komponen ditutup
		return () => {
	...
	return (
		<section className='relative flex min-h-screen w-full flex-col items-center space-y-10 overflow-hidden bg-gray-100 pt-20 font-inter'>
			{/* NavBar, mengarahkan activePage ke 'Profil Desa' */}
			...
			<div className='flex max-w-[80%] flex-col space-y-4 rounded-xl md:flex-row md:space-x-4'>
				<div className='relative flex w-full flex-col space-y-4 rounded-xl border bg-white px-6 py-6 shadow-xl md:w-[50%]'>
					{/* Gambar Leaf dengan posisi absolute */}
					<img
						src={Leaf}
						alt='Leaf'
			...
			{/* Konten tentang Sejarah Desa Kedisan */}
			<div
				className='relative min-h-24 w-[80%] space-y-3 rounded-xl bg-white px-6 py-6 pb-16 text-justify shadow-xl'
				style={{ textIndent: '2rem' }} // Menambahkan indentasi pada teks paragraf
			>
				{/* Gambar Greek dan Scroll dengan posisi absolute */}
				<img
					src={Greek}
					alt='Greek'
				...
				<p
					...
					style={{ textIndent: '0' }} // Menghilangkan indentasi pada teks judul
				>
			...
			{/* Konten tentang Visi dan Misi Desa Kedisan */}
			<div className='flex w-[80%] flex-col space-y-5'>
				<div className='flex w-full flex-row overflow-hidden rounded-xl bg-white shadow-xl'>
					<div className='flex w-[20%] items-center justify-center bg-gradient-to-br from-custred to-custorange text-3xl font-extrabold tracking-widest text-white md:px-4'>
						{/* Memutar teks jika ukuran layar lebih kecil dari medium */}
			...
			{/* Konten tentang Struktur Pemerintahan Desa Kedisan */}
			<div className='flex w-[80%] flex-col items-center space-y-5'>
				<div className='flex h-64 w-48 flex-col items-center space-y-3 overflow-hidden rounded-xl bg-white pt-3'>
					...
					<div className='flex w-full flex-grow flex-col items-center justify-center space-y-0 bg-custred px-1 text-white'>
						{/* Mengambil data dari file JSON strukturPerbekel */}
				...
				{/* Elemen Swiper untuk menampilkan struktur pemerintahan */}
				<Swiper
					modules={[Navigation]}
				...
				>
					{/* Mengambil data dari file JSON strukturPerbekel menggunakan Object.entries agar bisa di-iterate */}
					{Object.entries(strukturPerbekel.Kelian_Banjar_Dinas).map(([key, value]) => (
						// Setiap data akan ditampilkan dalam SwiperSlide
						<SwiperSlide key={key}>
							<div className='flex w-auto items-center justify-center'>
								<div className='flex h-64 w-48 flex-col items-center space-y-3 overflow-hidden rounded-xl bg-white pt-3'>
									...
									<div className='flex w-full flex-grow flex-col items-center justify-center space-y-0 bg-custred px-2 text-white'>
										...
										<p className='text-center text-xs'>{key.replace(/_/g, ' ')}</p> {/* Mengganti underscore dengan spasi */}
									</div>
								</div>
							</div>
						</SwiperSlide>
				...
				<div className='!mt-12 flex h-64 w-48 flex-col items-center space-y-3 overflow-hidden rounded-xl bg-white pt-3'>
					<PersonIcon />
					<div className='flex w-full flex-grow flex-col items-center justify-center space-y-0 bg-custred px-1 text-white'>
						{/* Mengambil data dari file JSON strukturSekretariat */}
				...
				{/* Elemen Swiper untuk menampilkan struktur sekretariat */}
...
// Komponen untuk menampilkan icon orang
const PersonIcon = () => (
